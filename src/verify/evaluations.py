...
def is_equiv(ground_truth: str, given: str, verbose: bool = False, fast: bool = False):
    ...
    def default_is_equiv(
        ground_truth_normalized: Union[str, set], given_normalized: Union[str, set]
    ) -> bool:
        if verbose:
            print(ground_truth_normalized, given_normalized)

        if (
            isinstance(ground_truth_normalized, str)
            and ground_truth_normalized == given_normalized
        ):
            return True
...
def is_latex_equiv(
    ...
    if not is_correct and (
        _str_is_mat(ground_truth_normalized) or _str_is_mat(given_normalized)
    ):
...
def is_equiv_possible_intervals(
    ...
    if (
        ...
        and gv_interval is not None
        and gt_interval.compare(gv_interval) == 0
    ):
...
def is_equiv_possible_tuple(
    ...
    if isinstance(ground_truth_elems, str) and isinstance(given_elems, str):
        if (
            ground_truth_elems == ground_truth_normalized
            and given_elems == given_normalized
        ):
            return False, False
        else:
...
def is_equiv_possible_matrix(
    ...
    if isinstance(gt_matrix, str) and isinstance(gv_matrix, str):
        ...
        for gt_col, gv_col in zip(gt_matrix, gv_matrix):
            if (
                isinstance(gt_col, str)
                and isinstance(gv_col, str)
                and is_equiv(gt_col, gv_col)
            ):
                continue
