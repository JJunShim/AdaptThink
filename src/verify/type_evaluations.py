...
from .evaluations import (
    is_equiv,
    is_equiv_possible_matrix,
    is_equiv_possible_tuple,
    is_latex_equiv,
)
...
def is_equiv_complex(
    ...
    grt_normalized = string_normalize(ground_truth, lower_case=False).replace(
        "\\sqrt{-1}", "I"
    )
    pre_normalized = string_normalize(prediction, lower_case=False).replace(
        "\\sqrt{-1}", "I"
    )
