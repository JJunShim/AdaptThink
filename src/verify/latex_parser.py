...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="latex2sympy_wrapper", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="parse_latex_wrapper", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="parse_expr_wrapper", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="is_sympy_zero", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="get_sympy_type", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="list_to_mat", seconds=TIMEOUT_SECONDS
    ),
)
def list_to_mat(list_expr, exp_rows, exp_cols):
    try:
        ...
        if mat.rows == exp_rows and mat.cols == exp_cols:
            return mat
        elif (exp_rows == 1 and mat.rows == exp_cols) or (
            exp_cols == 1 and mat.cols == exp_rows
        ):
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="ineq_to_expr", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="uneq_to_set", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="expr_with_only_i", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="sympy_simplify", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="sympy_expand_equal", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="sympy_abs", seconds=TIMEOUT_SECONDS
    ),
)
...
@timeout(
    TIMEOUT_SECONDS,
    exception_message=TIMEOUT_MESSAGE.format(
        func_name="sympy_evalf", seconds=TIMEOUT_SECONDS
    ),
)
def sympy_evalf(
    sympy_expr: Expr,
...
def _diff_equal(
    ...
    def abs_smaller_than_threshold(latex_sympy, threshold: float) -> bool:
        try:
            ...
            return (
                abs_latex_sympy < threshold or sympy_evalf(abs_latex_sympy) < threshold
            )
        except:
            return False
...
def _numerical_equal(
    ...
    for _ in range(10):
        ...
        if (
            temp_gt is None
            or temp_gv is None
            or not _diff_equal(temp_gt, temp_gt - temp_gv)
        ):
            flag = False
            break
    return flag
...
def _are_equal_latex_sympy(
    ...
    if gt_type == gv_type == ANSWER_TYPE_INEQ:
        if (
            ...
            gt_latex_sympy.rel_op in {"<", ">"} and gv_latex_sympy.rel_op in {"<", ">"}
        ):
    ...
    if gt_type == gv_type == ANSWER_TYPE_CPLX:
        ...
        return _are_equal_latex_sympy(
            gt_re, gv_re, gt_re_type, gv_re_type
