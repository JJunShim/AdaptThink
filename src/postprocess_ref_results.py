...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, default="deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"
    )
    parser.add_argument(
        "--input_path",
        type=str,
        default="./data/train/ref_presampling/DeepSeek-R1-Distill-Qwen-1.5B_deepscaler_n10_K16_len16384.jsonl",
    )
...
def process(id):
    ...
    # 정확도 계산
    correctness = [
        adapt_think_rm(data_source="", solution_str=sol, ground_truth=real_answer).get(
            "acc"
        )
...
if __name__ == "__main__":
    ...
    # 전체 메트릭 계산
    metric_keys = results[0].get("metrics").keys()
    overall_metrics = {
        key: np.mean([r.get("metrics").get(key) for r in results])
        for key in metric_keys
    }

    # 전체 메트릭을 결과 앞에 추가
    results.insert(
        0, {"problem": "__OVERALL__", "answer": None, "metrics": overall_metrics}
    )
