...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == "bpo":
        advantages, returns = adapt_think_core_algos.compute_batchwise_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
        )
...
def compute_adapt_think_metrics(batch):
    ...
    metrics["adapt_think/nothinking_reward-thinking_reward/mean"] = (
        metrics["adapt_think/nothinking_reward/mean"]
        - metrics["adapt_think/thinking_reward/mean"]
    )
    print(f"\n\nadapt_think_METRICS\n{metrics}\n\n")
    return metrics
...
class AdaptThinkRayPPOTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for data_source, results in data_src2val_results.items():
            json.dump(
                results,
                open(f"{save_dir}/data/{data_source}.json", "w"),
                indent=2,
                ensure_ascii=False,
            )
        json.dump(
            metric_dict,
            open(f"{save_dir}/overall_metrics.json", "w"),
            indent=2,
            ensure_ascii=False,
        )
