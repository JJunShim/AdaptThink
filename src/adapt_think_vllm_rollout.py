...
class AdaptThinkvLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, adapt_think_config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        max_model_len = self.config.max_model_len if self.config.max_model_len \
                        else config.prompt_length + max(config.response_length, config.val_kwargs.max_tokens)
        ...
        self.nothinking_max_len = adapt_think_config.nothinking_max_response_length
        self.eot_token_id = adapt_think_config.eot_token_id
        self.non_eot_token_id = adapt_think_config.non_eot_token_id
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        assert do_sample, "do_sample must be True"
        if is_validate:
            # TODO: try **
            kwargs = {
        ...
        with self.update_sampling_params(**kwargs):
            if is_validate:
                ...
                if n_nothinking > 0:
                    ...
                    sampling_params_nothinking.max_tokens = self.nothinking_max_len - 1
                    vllm_inputs_nothinking = deepcopy(vllm_inputs)
                    for i, ipt in enumerate(vllm_inputs_nothinking):
                        ipt['prompt_token_ids'] = ipt['prompt_token_ids'] + [self.eot_token_id]
                ...
                if n_thinking > 0:
                    ...
                    for i, ipt in enumerate(vllm_inputs_thinking):
                        ipt['prompt_token_ids'] = ipt['prompt_token_ids'] + [self.non_eot_token_id]
                    ...
                    outputs_thinking = [[],]*len(vllm_inputs)


                response = []
                enforce_nothinking = []
                assert len(outputs_nothinking) == len(outputs_thinking), f"{len(outputs_nothinking)} != {len(outputs_thinking)}"
                for output_thinking, output_nothinking in zip(outputs_nothinking, outputs_thinking):
                    if output_thinking != []:
                        for sample_id in range(len(output_thinking.outputs)):
                            enforce_nothinking.append(True)
                            response.append([self.eot_token_id] + output_thinking.outputs[sample_id].token_ids)
            ...
            enforce_nothinking = torch.tensor(enforce_nothinking).to(idx.device)
